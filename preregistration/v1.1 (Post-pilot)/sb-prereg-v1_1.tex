...
\date{\today\\
	Post-Pilot Version (v1.1) with Addendum}
...
\clearpage
\section{Post-Pilot Addendum}
\label{App:PostPilotAddendum}

This section documents refinements made after pilot testing and prior to the
start of the main data collection. These amendments are restricted to
implementation details, measurement improvements, and clarifications that
do not alter the preregistered research questions, estimands, hypotheses,
or inferential criteria. Pilot analyses were conducted solely to verify
computational feasibility, data recording, and model executability.
Behavioral pilot statistics (e.g., betting rates, sequence-level means,
or treatment differences) were not used to modify hypotheses, estimands,
or decision rules.

Prior to the start of the main data collection, the preregistered analysis
scripts implementing the statistical procedures described in this document
were prepared and uploaded to the project Git repository. The repository
serves as a version-controlled record of the analysis pipeline used for
the confirmatory and exploratory analyses reported in the study.

\subsection{Stan Diagnostics and Sampler Configuration}

All models will be monitored using standard Stan diagnostics,
including divergences, treedepth saturation, $\hat{R}$, and effective
sample size (ESS). If diagnostics indicate poor posterior geometry
(e.g., divergences $>0$ or widespread treedepth saturation),
sampler tuning parameters will be adjusted (e.g., increasing
\texttt{adapt\_delta} or \texttt{max\_treedepth}), and non-centered
parameterizations will be used where appropriate until diagnostics
indicate satisfactory sampling behavior.

These adjustments affect only numerical sampling performance and do
not alter the model likelihoods, priors, estimands, or reporting rules.

\subsection{Holt--Laury Risk-Elicitation Task}

\paragraph{Extended Holt--Laury list.}

To improve identification of individual CRRA parameters used in
downstream expected-utility calculations, the Holt--Laury list
will contain 20 probability steps rather than the standard 10.
Lottery probabilities increase in increments of 0.05 from 0.05
to 0.95 (corresponding to probabilities \(1/20,\dots,19/20\)).
The certainty row with probability 1.00 is omitted to avoid a
dominated choice that does not provide information about risk
preferences.

\paragraph{Consistency diagnostic.}

As a diagnostic of response quality, an inconsistency indicator
will be defined for each participant:
\[
\text{HL-inconsistent}_i =
\begin{cases}
	1 & \text{if a participant switches from risky to safe in a later row}, \\
	0 & \text{otherwise.}
\end{cases}
\]

Primary analyses will include all participants.
As a robustness check, all analyses involving estimated risk preferences
$r_i$ will be repeated excluding HL-inconsistent participants.

\paragraph{Treatment-specific estimation of risk preferences.}

The preregistration described the estimation of risk preferences using a
hierarchical Bayesian model applied to the Holt--Laury (HL) task.
During implementation it became clear that the hierarchical terminology
was imprecise relative to the intended estimation procedure.

In the implemented analysis, individual risk preferences are estimated
using a Bayesian HL choice model with informative priors on the
participant-level parameters. Each participant has an individual CRRA
risk parameter $r_i$ and a choice precision parameter $\lambda_i$,
with priors
\[
r_i \sim \mathcal{N}(0.27,\,0.36),
\qquad
\lambda_i \sim \mathrm{Lognormal}(\log 30,\,0.5),
\]
following the calibration suggested in Bland (2023).
These priors are fixed and no higher-level population parameters are
estimated; the model therefore performs individual-level Bayesian
estimation rather than hierarchical partial pooling.

Because the HL task is administered after the sequence experiment and may
itself be affected by prior exposure to the incentive frame, risk
preferences will be estimated separately for each treatment group.
The Bayesian HL model will therefore be fitted independently for the
FN and FP samples using identical likelihoods, priors, and MCMC settings.

Estimating the model separately by treatment ensures that the
participant-level posterior draws $r_i^{(m)}$ reflect only
within-treatment information and are not influenced by the distribution
of responses in the other treatment.

As an exploratory diagnostic, we will additionally report the correlation
between treatment-specific estimates of $r_i$ and estimates obtained
from a pooled HL model fitted to the full sample.

\subsection{Computational Implementation of Risk-Parameter Propagation}

The preregistration specifies that posterior uncertainty in $r_i$ should
be propagated to derived quantities such as $a^*(r_i)$ and certainty
equivalents. Pilot analyses indicated that fully integrating the
posterior draws of $r_i$ inside the sequence-level Bayesian models
substantially increases computational complexity.

To maintain computational feasibility while preserving partial
propagation of uncertainty from the HL task, derived quantities that
depend on risk preferences are computed from posterior draws of $r_i$
outside the sequence-level models and summarized at the participant
level.

Specifically, expected-utility--optimal stakes are first computed for
each posterior draw of the risk parameter, yielding $a^*(r_i^{(m)})$.
The participant-level benchmark used in the sequence analyses is then
defined as the posterior mean of this derived quantity:
\[
\tilde a_i^* = E[a^*(r_i)\mid \text{HL data}].
\]

Analogously, certainty-equivalent quantities that depend on $r_i$ are
computed draw-wise and subsequently summarized before entering the
sequence-level analyses.

This approach preserves uncertainty propagation at the level of the
derived benchmark while avoiding full joint estimation of the HL and
sequence models. Sensitivity checks will recompute the relevant
quantities using alternative posterior summaries (e.g., medians or
selected posterior draws) to verify that substantive conclusions are not
sensitive to this implementation choice.

\subsection{Clarification of RQ2 Benchmark Interpretation}

Although the normative benchmark implies
$\mu_s^{a} \approx 0$, empirically the MPL-implied risk parameter $r_i$
may not fully transport to the betting task. Factors such as probability
weighting or task-specific framing may generate systematic under- or
over-staking across all sequences.

Such global shifts are not interpreted as evidence for sequence-specific
distortions. Inference in RQ2 therefore focuses on between-sequence
differences in $\mu_s^{a}$ and on sequence-specific posterior
concentration outside the ROPE conditional on the MPL-based benchmark.

\paragraph{Relative sequence contrasts.}

To separate global calibration shifts from sequence-specific effects,
we additionally report descriptive relative sequence contrasts based on
the hierarchical sequence effects $\beta_s$ from the RQ2 model.

Because these effects are identified under the constraint
$\sum_s \beta_s = 0$, they represent deviations from the grand mean
across sequences. Posterior medians and 95\% credible intervals of
$\beta_s$ will therefore be reported descriptively.

Sequences whose posterior intervals exclude zero will be described
as exhibiting credible relative shifts. These contrasts are exploratory
and do not alter the preregistered inferential criteria based on the
expected-utility benchmark.

\subsection{Clarification of RQ4 Population Baseline}

For clarity, the baseline directional probability $\bar{h}$ and the
sequence-level means $\mu_s^{h}$ are defined as population expectations
with respect to the participant-level heterogeneity distribution implied
by the hierarchical model:
\[
\bar{h} =
E_{u \sim \mathcal N(0,\sigma_u)}
\!\left[\operatorname{logistic}(\alpha + u)\right],
\]
\[
\mu_s^{h} =
E_{u \sim \mathcal N(0,\sigma_u)}
\!\left[\operatorname{logistic}(\alpha + u + \beta_s)\right].
\]

These expectations integrate over participant heterogeneity and ensure
that both baseline and sequence-level quantities are defined on the same
population scale.

\subsection{Propagation of posterior uncertainty in sequence classification in EX1}

The preregistration describes sequence similarity probabilities
$d_a(s)=P(|\theta_s-\theta_a|<\epsilon\mid\text{data})$ and the
corresponding normalized weights $w_a(s)$ as posterior summaries used
to construct the directional score $z_{is}$. In that formulation the
similarity probabilities are first collapsed across posterior draws
before computing $z_{is}$ and the derived indices $\chi_s$ and $\chi_i$.

During pilot implementation it became clear that collapsing the
similarity probabilities at this stage would produce deterministic
directional indices and would therefore not support the probabilistic
classification of sequences and participants (e.g.,
$H(\cdot)=P(\chi_s>\delta)$, $T(\cdot)=P(\chi_s<-\delta)$, and
$HH(\cdot),G(\cdot)$).

To maintain probabilistic classification while preserving the
anchor-based interpretation of the directional score, the implemented
analysis propagates posterior uncertainty through the directional index
construction. Similarity indicators and normalized weights are computed
within each posterior draw, yielding draw-specific directional scores
$z_{is}^{(k)}$. Sequence-level and participant-level indices,
$\chi_s^{(k)}$ and $\chi_i^{(k)}$, are then obtained at the draw level
and summarized using posterior medians, credible intervals, and the
probabilities $H(\cdot)$, $T(\cdot)$, $HH(\cdot)$, and $G(\cdot)$.

This modification preserves the conceptual framework of anchor-based
directional similarity while ensuring that the preregistered
probabilistic classification rules are well defined.

\subsection{Treatment coverage in EX2}

EX2 was originally described for the confirmatory treatment only.
Because EX2 examines exploratory associations between participant
characteristics and behavioral outcomes, the analysis will instead
be estimated separately for each treatment using the full sample
within that treatment.

\subsection{Additional Implied quantities and contrasts in EX3}

\paragraph{Implied probability diagnostics.}

Pilot data indicated higher betting rates in the FP treatment than
initially expected. To assess whether these stakes reflect systematic
belief distortions, the implied probability diagnostic described in
EX3 will additionally be computed for the FN treatment.
For each betting trial ($a_{is}>0$), the implied subjective probability
$\hat p_{is}$ is obtained by solving the first-order condition of the
CRRA expected-utility model. FN results are reported descriptively and
do not alter the confirmatory analyses.

\paragraph{Implied risk-aversion diagnostic.}

As an additional internal consistency check, we compute an implied
risk-aversion parameter $r^\ast$ that rationalizes the observed stake
under alternative belief assumptions:
\begin{itemize}
	\item objective probability $p=0.5$,
	\item the baseline probability estimated in RQ4,
	\item a self-reported belief collected in the post-experiment questionnaire.
\end{itemize}

Participant-level summaries of $r^\ast$ are compared descriptively
with the HL-based estimates of $r_i$.

\paragraph{Additional FN--FP contrasts.}

Because pilot data suggest sufficient betting in the FP treatment,
descriptive FN--FP contrasts will also be reported for RQ2 and RQ4.
These contrasts follow the same reporting conventions as for RQ1 and RQ3
(posterior medians, credible intervals, and posterior probabilities
of direction).

\paragraph{Participant-level contrasts.}

Because treatments are assigned between subjects, participant-level
contrasts are implemented at the group level. For each posterior draw,
we compute treatment-level averages of participant-level summaries and
...
\subsection{Additional Debriefing Questions}

Pilot observations indicated unexpectedly frequent betting in the
lower-multiplier treatment. To aid interpretation of this behavior,
two additional debriefing questions are included:

\begin{enumerate}
	\item participants’ subjective estimate of how often a bet would win
	(out of 100 rounds),
	\item their overall perception of the riskiness of betting in the task.
\end{enumerate}

These questions provide descriptive information about participants’
beliefs and perceptions and do not affect the preregistered analyses.
